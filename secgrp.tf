...
resource "aws_vpc_security_group_ingress_rule" "ssh_from_anywhere_to_bastion" {
  security_group_id = aws_security_group.bastion_sg.id
  cidr_ipv4         = "0.0.0.0/0"
  from_port         = 22
  ip_protocol       = "tcp"
...
resource "aws_security_group" "stefan_rds_sg" {
  name        = "stefan_rds_sg"
...
resource "aws_vpc_security_group_ingress_rule" "allow_app_to_mysql" {
  security_group_id            = aws_security_group.stefan_rds_sg.id
  referenced_security_group_id = aws_security_group.app_sg.id
  from_port                    = 3306
  ip_protocol                  = "tcp"
  to_port                      = 3306
